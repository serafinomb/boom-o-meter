...
const FlashReachCalculator = () => {
  ...
  const SettingsScreen = () => (
    <div className="max-w-lg mx-auto bg-gray-100 p-6 rounded-2xl shadow-lg font-mono">
      ...
      <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg p-4 shadow-inner space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-xs font-semibold text-gray-800">{getSliderLabel(settings.priorityWeights.accuracy)}</span>
              </div>
              <CustomSlider
                min={0}
                max={1}
                ...
                onChange={(e) => setSettings({
                  ...settings,
                  priorityWeights: {...settings.priorityWeights, accuracy: parseFloat(e.target.value)}
                })}
                color="blue"
              />
            </div>
        ...
        {/* Battery Saving Mode */}
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-3">Battery Saving</label>
          <div className="bg-white rounded-lg p-4 shadow-inner">
            <label className="flex items-center space-x-3 cursor-pointer">
              <input
                type="checkbox"
                checked={settings.batterySavingMode}
                onChange={(e) => setSettings({...settings, batterySavingMode: e.target.checked})}
                className="rounded"
              />
              <div>
                <div className="text-sm font-medium">Enable Battery Saving Mode</div>
                <div className="text-xs text-gray-600">Excludes high power settings (1/1, 1/2)</div>
              </div>
            </label>
          </div>
        </div>
  ...
  const getSolutionBadge = (solution, index) => {
    ...
    return (
      ...
      >
        {indicatorColor && (
          <div className={`absolute top-2 right-2 w-3 h-3 rounded-full ${indicatorColor}`}></div>
        )}
  ...
  const CalculatorScreen = () => (
    <div className="max-w-lg mx-auto bg-gray-100 p-6 rounded-2xl shadow-lg font-mono">
      ...
      <div className="mb-6">
        ...
        <div className="bg-white rounded-lg p-4 shadow-inner">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm text-gray-500">{settings.minDistance.toFixed(1)}m</span>
            <div className="text-2xl font-bold text-blue-600">
              {targetDistance.toFixed(1)}m
            </div>
            <span className="text-sm text-gray-500">{settings.maxDistance.toFixed(1)}m</span>
          </div>
          <CustomSlider
      ...
      {settings.photographyMode === 'digital' && (
        <div className="mb-6">
          <label className="block text-xs text-gray-600 mb-2 uppercase tracking-wide">ISO</label>
      ...
      <div className="mb-6">
        <label className="block text-xs text-gray-600 mb-3 uppercase tracking-wide">
          Suggested Settings {viableSolutions.length > 0 && `(${viableSolutions.length} options)`}
        </label>
        {viableSolutions.length > 0 ? (
          <>
            <div className="grid grid-cols-2 gap-2 mb-3">
              {viableSolutions.map((solution, index) => getSolutionBadge(solution, index))}
            </div>
            {/* Legend */}
          ...
          <div className="text-center py-8 text-gray-500">
            ...
            <div className="text-xs mt-1">Try adjusting distance, settings, or ISO</div>
          </div>
        )}
      </div>
      ...
      {currentSolution && (
        <div className="mb-4 p-4 bg-white rounded-lg shadow-inner">
          <div className="text-center">
            <div className="text-lg font-bold text-gray-800 mb-2">Selected Settings</div>
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="text-xs text-gray-500 uppercase">Aperture</div>
                <div className="text-xl font-bold text-orange-600">f/{currentSolution.aperture}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500 uppercase">Power</div>
                <div className="text-xl font-bold text-green-600">{currentSolution.power}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500 uppercase">Distance</div>
                <div className="text-xl font-bold text-blue-600">{currentSolution.actualDistance.toFixed(1)}m</div>
              </div>
            </div>
      ...
      <div className="mb-6 p-4 bg-white rounded-lg shadow-inner">
        ...
        <div className="text-xs text-gray-500 text-center mt-2">
          ISO {currentISO} • GN {settings.guideNumber} • Based on your configured settings
        </div>
      </div>
      ...
      <div className="flex justify-between items-center">
        <div className="text-center flex-1">
          ...
          <div className="text-xs text-gray-500">Distance-First Flash Calculator</div>
        </div>
        <button
  ...
  return currentScreen === 'settings' ? <SettingsScreen /> : <CalculatorScreen />;
};
